...
func writeEmbeddedConfigSource(buildDir string, data []byte) error {
	quoted := strconv.Quote(string(data))
	src := fmt.Sprintf(`// Code generated by internal/assets/packer. DO NOT EDIT.

package build

var embeddedConfig = []byte(%s)

// EmbeddedConfig returns the JSON configuration baked into the binary.
func EmbeddedConfig() []byte {
...
`, quoted)
	...
	if err != nil {
		return fmt.Errorf("format embedded config: %w", err)
	}

	path := filepath.Join(buildDir, "config_data.go")
	if err := os.WriteFile(path, formatted, 0o644); err != nil {
		return fmt.Errorf("write config_data.go: %w", err)
	}

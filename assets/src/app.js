...
function toggleHeader() {
  ...
  if (isHeaderCollapsed) {
    ...
    collapseBtn.classList.add(
      "bi-list",
      headerWhiteBg ? "primary-text-color" : null,
    );
...
const setFeedback = (message, type = "success") => {
  ...
  feedback.classList.remove("text-rose-600", "text-emerald-600");
...
if (form) {
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      setFeedback("Thanks! Your message is on the way.", "success");
      form.reset();
    } catch (error) {
